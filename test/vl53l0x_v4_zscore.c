...
#define   rawDataFrameNum     3
#define   bgFrameNum          30

static uint8_t  rawDataFrameIndex           = 0;
static float    rawData[64*rawDataFrameNum] = {0};
static uint8_t  bgFrameIndex                = 0;
static float    bgData[64*bgFrameNum]  = {0};

static bool isCalibrate = false;
static float avgCal[64] = {0};
static float stdCal[64] = {0};
static float zscoreData[64*rawDataFrameNum] = {0};
static float zscore[64] = {0};


static void labelPixel(float* pixelTemp, uint8_t* color);
...
void calibration() {
  for(int i=0; i<64; i++) {
    float tempSum = 0;
    for(int j=0; j<bgFrameNum; j++) {
      tempSum += bgData[i+j*64];
    }
    avgCal[i] = tempSum / bgFrameNum;
  }
  ...
  for(int i=0; i<64; i++) {
    ...
    stdCal[i] = sqrtf(tempSum/ bgFrameNum);
  }
}
...
void zscoreCalculation() {
  for(int i=0; i<64; i++) {
    float tempSum = 0;
    for(int j=0; j<rawDataFrameNum; j++) {
      tempSum += rawData[i+j*64];
    }
    float sampleAvg = tempSum / rawDataFrameNum;
    int map_index = i + (64*rawDataFrameIndex);
    ...
    zscore[i] = tempSum / rawDataFrameNum;
  }
}

void labelPixel(float* pixelTemp, uint8_t* color)
{
    for(int i=0; i<64; i++){
...
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[0])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[1])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[2])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[3])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[4])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[5])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[6])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[7])
LOG_GROUP_STOP(gridEyeR0)

LOG_GROUP_START(gridEyeR1)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[8])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[9])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[10])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[11])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[12])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[13])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[14])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[15])
LOG_GROUP_STOP(gridEyeR1)

LOG_GROUP_START(gridEyeR2)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[16])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[17])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[18])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[19])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[20])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[21])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[22])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[23])
LOG_GROUP_STOP(gridEyeR2)

LOG_GROUP_START(gridEyeR3)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[24])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[25])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[26])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[27])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[28])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[29])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[30])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[31])
LOG_GROUP_STOP(gridEyeR3)

LOG_GROUP_START(gridEyeR4)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[32])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[33])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[34])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[35])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[36])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[37])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[38])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[39])
LOG_GROUP_STOP(gridEyeR4)

LOG_GROUP_START(gridEyeR5)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[40])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[41])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[42])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[43])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[44])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[45])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[46])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[47])
LOG_GROUP_STOP(gridEyeR5)

LOG_GROUP_START(gridEyeR6)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[48])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[49])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[50])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[51])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[52])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[53])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[54])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[55])
LOG_GROUP_STOP(gridEyeR6)

LOG_GROUP_START(gridEyeR7)
LOG_ADD(LOG_UINT16, rawDataC0, &zscore[56])
LOG_ADD(LOG_UINT16, rawDataC1, &zscore[57])
LOG_ADD(LOG_UINT16, rawDataC2, &zscore[58])
LOG_ADD(LOG_UINT16, rawDataC3, &zscore[59])
LOG_ADD(LOG_UINT16, rawDataC4, &zscore[60])
LOG_ADD(LOG_UINT16, rawDataC5, &zscore[61])
LOG_ADD(LOG_UINT16, rawDataC6, &zscore[62])
LOG_ADD(LOG_UINT16, rawDataC7, &zscore[63])
